...
class ItemsController extends Controller
{
    ...
    public function editAction(Request $request, items $item)
    {
        ...
        if($user == null)
        {
            ...
            $deleteForm = $this->createDeleteForm($item);
            $editForm = $this->createForm('ItemsBundle\Form\itemUpdateType', $item);
            $editForm->handleRequest($request);

            if ($editForm->isSubmitted()) {
               // die('ced. '.$item->getNomitems());
                $this->getDoctrine()->getManager()->flush();

                return $this->redirectToRoute('items_show', array('iditems' => $item->getIditems()));
            }

            return $this->render('@Items/items/edit.html.twig', array(
                'item' => $item,
    ...
    public function deleteAction(Request $request, items $item)
    {
        ...
        if($user == null)
        {
            ...
            return $this->redirectToRoute('items_homepage');
        }
    }
